...
int iris_parse_param(struct dsi_display *display)
{
	...
	if (!strcmp(display->panel->name, "BOE AB319 NT37701B UDC")
		|| !strcmp(display->panel->name, "tianma nt37705 dsc cmd mode panel"))
		lightup_node = of_parse_phandle(display->pdev->dev.of_node, "pxlw,iris-lightup-config", 0);
	else if (!strcmp(display->panel->name, "BOE AB241 NT37701A"))
		lightup_node = of_parse_phandle(display->pdev->dev.of_node, "pxlw,iris-lightup-config-1", 0);
