...
static int iris_frc_label;
static int iris_frc_var_disp;   //0: disable, 1: set before FRC, 2: set after FRC
// bit[4], enable sw configure, bit[3:0], sw motion level, 0-4
static int iris_frc_mnt_level;
...
static int iris_frc_fallback_disable;
...
static int iris_frc_osd_protection;
...
static int iris_firep_force_en = 1;
static int iris_debug_trace_en = 0;
static int iris_check_pwil_intstat;
...
static void iris_rfb_mode_enter(bool bRFB)
{
	...
	if (bRFB)
		pcfg->pwil_mode = RFB_MODE;
	IRIS_LOGI("enter %s, rx_mode: %d, tx_mode: %d",
		pcfg->pwil_mode == PT_MODE ? "PT" : "RFB", pcfg->rx_mode, pcfg->tx_mode);
	if (pcfg->osd_enable)
...
static void iris_pt_mode_enter(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();

	if (pcfg->rx_mode != pcfg->tx_mode) {
		IRIS_LOGI("cannot enter PT, rx_mode: %d, tx_mode: %d", pcfg->rx_mode, pcfg->tx_mode);
		return;
	}

	pcfg->pwil_mode = PT_MODE;
	IRIS_LOGI("enter PT, rx_mode: %d, tx_mode: %d", pcfg->rx_mode, pcfg->tx_mode);
	iris_set_pwil_mode(pcfg->panel, pcfg->pwil_mode, pcfg->osd_enable, DSI_CMD_SET_STATE_HS);
}
...
static u32 iris_get_vtotal(void)
{
	...
	uint32_t vtotal;
	struct iris_cfg *pcfg = iris_get_cfg();
	bool use_2nd_tm = iris_belongs_to_2nd_timing(&pcfg->panel->cur_mode->timing);
...
void iris_frc_mif_reg_set(void)
{
	...
	u32 iris_game_mode = pcfg->frc_app_info & 0x01;
	...
	fmif_dsy_hstride = (((frc_setting->mv_hres + 1) / 2) * 8 + 63) / 64;
	fmif_dsy_frm_offset = fmif_dsy_hstride * (frc_setting->mv_vres * 5 / 10) * 8;
	...
	if (frc_setting->disp_hres != frc_setting->mv_hres)
		start_line = 5;
	else
		start_line = 0;
	...
	case 45:
		if (pcfg->frc_low_latency) {
			...
			three_metagen_en = 0;
			frc_ratio = 0x801;
		} else if (frc_setting->out_fps == 90) {
			three_mvc_en = 1;
			three_metagen_en = 0;
			frcc_teadj_th = 0x288080;
			phase_teadj_en = 0;
			frc_ratio = 0x801;
		}
		break;
	case 60:
		if (pcfg->frc_low_latency) {
			...
			three_mvc_en = 0;
			three_metagen_en = 0;
		} else if (frc_setting->out_fps == 120 && iris_game_mode) {
			three_mvc_en = 1;
			three_metagen_en = 0;
			frcc_teadj_th = 0x288080;
			phase_teadj_en = 0;
	...
	if (pcfg->rx_mode == DSI_OP_CMD_MODE)
		...
		if (pcfg->prev_mode_rfb)
			iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG3, 0x58602104, 0);
		else
	...
	// game can't enable fallback function.
	if (!iris_game_mode)
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_TEOFFSET_CTRL, 0x00000060 | phase_teadj_en, 0);
	else
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_TEOFFSET_CTRL, 0x00000040 | phase_teadj_en, 0);
	...
	if (iris_frc_var_disp && !pcfg->osd_enable) {
		...
		//game can't enable PT+FRC mode
		if (iris_frc_pt_switch_on && (!iris_game_mode))
	...
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + IMIF_MODE_CTRL, 0x4 + ((frc_setting->mv_vres * 3 / 4) << 16), 0);
...
void iris_gmd_reg_set(void)
{
	...
	iris_frc_reg_add(IRIS_GMD_ADDR + GMD_STOP, frc_setting->mv_hres + (frc_setting->mv_vres << 16), 0);
	iris_frc_reg_add(IRIS_GMD_ADDR + GMD_CTRL, 0x00000011, 0);
}

void iris_fbd_reg_set(void)
{
	iris_frc_reg_add(IRIS_FBD_ADDR + FILMBD_RESOLUTION, (frc_setting->mv_hres / 2) +
							    ((frc_setting->mv_vres / 2) << 16), 0);
	iris_frc_reg_add(IRIS_FBD_ADDR + FILMBD_WIN_STOP_SET, frc_setting->mv_hres + (frc_setting->mv_vres << 16), 0);
...
void iris_mvc_reg_set(void)
{
	...
	//45->90 game mvc modify
	if (iris_game_mode &&
		((frc_setting->in_fps == 45 && frc_setting->out_fps == 90) ||
		(iris_game_mode && frc_setting->in_fps == 60 && frc_setting->out_fps == 120))) {
		iris_frc_reg_add(IRIS_MVC_ADDR + MVC_POSTKOCCADJ_0, 0x00000040, 0);
		iris_frc_reg_add(IRIS_MVC_ADDR + MVC_FGPROT_0, 0x10000210, 0);
		iris_frc_reg_add(IRIS_MVC_ADDR + MVC_POSTFILT_0, 0x22041009, 0);
		iris_frc_reg_add(IRIS_MVC_ADDR + MVC_POSTMVAVG_0, 0x08401000, 0);
		iris_frc_reg_add(IRIS_MVC_ADDR + MVC_POSTFILT_1, 0xffff00ff, 0);
	}
...
void iris_fi_reg_set(void)
{
	...
	if (iris_frc_demo_window == 1) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE, frc_setting->mv_hres << 16, 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE, frc_setting->mv_vres << 15, 0);
	} else if (iris_frc_demo_window == 2) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE, frc_setting->mv_hres << 16, 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE, frc_setting->mv_vres >> 1
				| frc_setting->mv_vres << 16, 0);
	} else if (iris_frc_demo_window == 3) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE, frc_setting->mv_hres << 15, 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE, frc_setting->mv_vres << 16, 0);
	} else if (iris_frc_demo_window == 4) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE, frc_setting->mv_hres >> 1
				| frc_setting->mv_hres << 16, 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE, frc_setting->mv_vres << 16, 0);
	...
	if (iris_game_mode)
		iris_frc_reg_add(IRIS_FI_ADDR + FI_BLENDING_CTRL2, 0x00208064, 0);
	//game protect gun center point
	iris_fi_demo_window_cal(false);
...
void iris_frc_mode_prepare(void)
{
	...
	iris_frc_reg_add(IRIS_PROXY_MB2, pcfg->frc_app_info, 0);
	if (pcfg->dual_setting) {
		iris_frc_reg_add(IRIS_PROXY_MB6, 0x12, 0);
	} else {
		if (iris_game_mode) {
			iris_frc_reg_add(IRIS_PROXY_MB6, 0x11, 0);
		} else {
			iris_frc_reg_add(IRIS_PROXY_MB6, 0x14, 0);
		}
	}
	/* FRC_MIF */
...
static void iris_blending_timeout_set(int frc) {
	struct iris_cfg *pcfg = iris_get_cfg();
	bool use_2nd_timing = iris_belongs_to_2nd_timing(&pcfg->panel->cur_mode->timing);

	if (frc) {
		iris_send_ipopt_cmds(IRIS_IP_BLEND, use_2nd_timing ? 0x31 : 0x11);	// blending: frc ocp
		iris_send_ipopt_cmds(IRIS_IP_BLEND, use_2nd_timing ? 0x30 : 0x10);	// blending: frc
	} else {
...
static void iris_pwil_intstat_check(void)
{
	u32 reg;
	struct iris_cfg *pcfg = iris_get_cfg();

	if (iris_check_pwil_intstat == 0)
		return;

	if (pcfg->pwil_abnormal)
		return;

	reg = IRIS_PWIL_ADDR + PWIL_INTSTAT_RAW;
	if (iris_i2c_ocp_read(&reg, 1, 0) < 0) {
		IRIS_LOGE("i2c read reg fails, reg=0x%x", reg);
		iris_check_pwil_intstat = 0;
		return;
	}

	IRIS_LOGI("PWIL int stat: %0x", reg);
	if (reg & 0x80) {
		IRIS_LOGE("PWIL PACK FLUSH");
		pcfg->pwil_abnormal = true;
	}
	if (reg & 0x20000) {
		IRIS_LOGE("PWIL BW FIFO ALMOST FULL");
		pcfg->pwil_abnormal = true;
	}
}

void iris_dtg_setting_update(bool lock)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	u32 *payload = NULL;
	u32 dtg_vsw, dtg_vbp, dtg_vres, dtg_vfp, dtg_vfp_max, dtg_vtotal;
	u32 ivsa_filter;
	u32 te_filter;
	u32 fi_preload_dly;
	u32 sw_fix_te_period;
	u32 ovs_dly_frc;
	u32 output_vtotal_change;
	u32 input_vtotal_change;
	u32 sw_n2m = 0;
	u32 cmd[50];
	int idx = 0;
	...
	dtg_vres = payload[6];
	dtg_vfp = payload[7];
	dtg_vfp_max = payload[7];
	ivsa_filter = payload[9];
	te_filter = payload[10];
	fi_preload_dly = payload[12];
	sw_fix_te_period = payload[23];
	ovs_dly_frc = payload[27];
	...
	IRIS_LOGI("default_panel_te: %d, panel_te: %d", pcfg->default_panel_te, pcfg->panel_te);
	...
	cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_7;
	cmd[idx++] = dtg_vfp;
	//cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_9;
	//cmd[idx++] = ivsa_filter;
	//cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_10;
	//cmd[idx++] = te_filter;
	cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_12;
	cmd[idx++] = fi_preload_dly;
	cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_19;
	cmd[idx++] = dtg_vfp_max + 20;	// VFP_MAX
	cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_20;
	cmd[idx++] = dtg_vfp_max + 20;	// VFP_MAX_RFB
	//cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_23;
	//cmd[idx++] = sw_fix_te_period;
	cmd[idx++] = IRIS_DTG_ADDR + DTG_REG_27;
	cmd[idx++] = ovs_dly_frc;
...
void iris_mode_switch_proc(u32 mode)
{
	...
	iris_pwil_intstat_check();
	if (pcfg->pwil_abnormal)
		return;
	IRIS_ATRACE_INT("mode_switch", mode);
	...
	if (mode == IRIS_MODE_FRC_PREPARE) {
		...
		if (!pcfg->osd_enable && pcfg->iris_osd_autorefresh_enabled) {
			IRIS_LOGW("osd_autorefresh is not disable when osd disable");
		}
		...
		//switch scaler filter for video or game
		...
		iris_rfb_mode_enter(pcfg->prev_mode_rfb);
	...
	} else if (mode == IRIS_MODE_RFB_POST || mode == IRIS_MODE_FRC_CANCEL) {
		if (pcfg->mcu_running) {
		...
		/*Power down BSRAM domain if when PT single channel and mipi2 power off*/
		if ((pcfg->pwil_mode == PT_MODE) && (pcfg->mipi2_pwr_st == false))
		...
		iris_rfb_mode_enter(true);
		pcfg->prev_mode_rfb = true;
	} else if (mode == IRIS_MODE_RFB2PT) {
		iris_pt_mode_enter();
		pcfg->prev_mode_rfb = false;
	} else if (mode == IRIS_MODE_PT_POST) {
		iris_pmu_bsram_set(false);
		iris_pmu_dscu_set(false);
	}
...
static int iris_get_mode(void)
{
	...
	if ((pcfg->dport_output_mode == 0) && ((pwil_mode == FRC_MODE && pcfg->switch_mode == IRIS_MODE_RFB2FRC)
		|| (pwil_mode == PT_MODE && pcfg->switch_mode == IRIS_MODE_FRC2RFB)))
...
void iris_fi_demo_window_cal(bool ocp_write)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	u32 temp0, temp1, temp2, temp3;
	u32 osd_window_enable[24] = {0xF2160038, 0x0, 0xF216003c, 0x0, 0xF2160040, 0x0,
					0xF2160044, 0x0, 0xF2160048, 0x0, 0xF216004c, 0x0,
					0xF2160050, 0x0, 0xF2160054, 0x0, 0xF2160058, 0x0,
					0xF216005c, 0x0, 0xF2160060, 0x0, 0xF217ff00, 0x1};
	u32 osd_window_disable[4] = {0xF2160060, 0x0, 0xF217ff00, 0x1};
	...
	if (pcfg->frc_setting.iris_osd_window_ctrl & 0x1249) { //osd protection
		if (!ocp_write) {
			...
			osd_window_enable[1] = osd0_tl;
			osd_window_enable[3] = osd1_tl;
			osd_window_enable[5] = osd2_tl;
			osd_window_enable[7] = osd3_tl;
			osd_window_enable[9] = osd4_tl;
			osd_window_enable[11] = osd0_br;
			osd_window_enable[13] = osd1_br;
			osd_window_enable[15] = osd2_br;
			osd_window_enable[17] = osd3_br;
			osd_window_enable[19] = osd4_br;
			osd_window_enable[21] = pcfg->frc_setting.iris_osd_window_ctrl;
			iris_ocp_write_mult_vals(24, osd_window_enable);
		}
	} else {
		if (!ocp_write) {
			iris_frc_reg_add(IRIS_FI_ADDR + FI_OSD_WINDOW_CTRL, 0x00000000, 0);
		} else {
			iris_ocp_write_mult_vals(4, osd_window_disable);
		}
	}
}
...
void iris_firep_force(bool enable)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	u32 val_frcc_reg8 = iris_val_frcc_reg8;
	u32 write_data[4];

	if (!iris_firep_force_en)
		return;
	if (pcfg->pwil_mode != FRC_MODE)
		return;
	if (enable)
		val_frcc_reg8 |= 0x40000000;
	write_data[0] = IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG8;
	write_data[1] = val_frcc_reg8;
	write_data[2] = IRIS_FRC_MIF_ADDR + FRCC_REG_SHDW;
	write_data[3] = 0x00000002;
	iris_ocp_write_mult_vals(4, write_data);
...
void iris_debug_mode_switch_set(int type, int value)
{
	switch (type) {
	case IRIS_DEBUG_MCU_ENABLE:
		iris_mcu_enable = value;
		break;
	case IRIS_DEBUG_FRC_VAR_DISPLAY:
		iris_frc_var_disp_dual = value;
		break;
	case IRIS_DEBUG_FRC_PT_SWITCH:
		iris_frc_pt_switch_on_dbg = value;
		break;
	case IRIS_DEBUG_TRACE_EN:
		iris_debug_trace_en = value;
	case IRIS_DEBUG_CHECK_PWIL:
		iris_check_pwil_intstat = value;
	default:
		break;
	}
}

void iris_debug_mode_switch_get(int type, int *value)
{
	switch (type) {
	case IRIS_DEBUG_MCU_ENABLE:
		*value = iris_mcu_enable;
		break;
	case IRIS_DEBUG_FRC_VAR_DISPLAY:
		*value = iris_frc_var_disp_dual;
		break;
	case IRIS_DEBUG_FRC_PT_SWITCH:
		*value = iris_frc_pt_switch_on_dbg;
		break;
	default:
		break;
	}
...
int iris_dbgfs_ms_init(struct dsi_display *display)
{
	...
	debugfs_create_u32("firep_force_en", 0644, pcfg->dbg_root,
			(u32 *)&iris_firep_force_en);
	debugfs_create_u32("trace_en", 0644, pcfg->dbg_root,
			(u32 *)&iris_debug_trace_en);
	debugfs_create_u32("check_pwil", 0644, pcfg->dbg_root,
			(u32 *)&iris_check_pwil_intstat);
	return 0;
}
