...
void iris_init(struct dsi_display *display, struct dsi_panel *panel)
{
	...
	pcfg->ext_clk = NULL; //devm_clk_get(&display->pdev->dev, "pw_bb_clk2");
	...
	if (!iris_virtual_display(display) && pcfg->valid >= PARAM_PARSED) {
		...
		psetting->quality_cur.cm_sdr2hdr_enable = 0;
	}
	_iris_get_vreg();
		...
		struct iris_ip_index *pip_index)
{
	...
	if (_iris_is_valid_dtsi_cmd_type(type)) {
		if (hdr->ip_type >= IRIS_IP_CNT) {
			IRIS_LOGE("hdr->i_p_type is  0x%0x out of max ip", hdr->ip_type);
			rc = -EINVAL;
...
static int iris_parse_iris_golden_fw(struct device_node *np)
{
	...
	rc = of_property_match_string(np, "iris-calibration-golden-fw-list", "iris5");
	if (unlikely(rc < 0)) {
		IRIS_LOGI("%s(), doesn't contain \"iris5\"", __func__);
		...
		rc = of_property_read_string_array(np, "iris-calibration-golden-fw-list",
			names_array, count);
		if (rc < 0) {
			IRIS_LOGI("Faild to invoke of_property_read_string_array()");
			goto default_fw_name;
		}
		if (index + 1 <= count) {
			pcfg->ccf1_name = names_array[index + 1];
			pcfg->ccf2_name = names_array[index + 2];
			IRIS_LOGI("%s(), name: %s %s", __func__, pcfg->ccf1_name, pcfg->ccf2_name);
		} else {
			rc = -1;
			goto default_fw_name;
		}
...
default_fw_name:
	pcfg->ccf1_name = "iris5_ccf1.fw";
	pcfg->ccf2_name = "iris5_ccf2.fw";
	return rc;
}
...
int iris_parse_param(struct dsi_display *display)
{
	...
	rc = iris_parse_iris_golden_fw(display->panel_node);
	if (rc < 0)
		IRIS_LOGI("can not parse golden fw name, using default %s %s",
				pcfg->ccf1_name, pcfg->ccf2_name);
	...
	iris_parse_loopback_info(lightup_node);
...
static int32_t _iris_i2c_s_send_ocp_cmds(struct dsi_panel *panel,
		struct iris_cmd_comp *pcmd_comp)
{
	int i = 0;
	bool is_burst;
	...
	if (!is_allburst) {
		for (i = 0; i < pcmd_comp->cnt; i++) {
			...
			is_burst = _iris_is_direct_bus(pcmd_comp->cmd[i].msg.tx_buf);
			if (is_burst) {
				IRIS_LOGE("%s: Not support burst.", __func__);
			} else {
	...
	IRIS_LOGE("[iris] %s:Not support allburst.", __func__);
	return -1;
}
		...
		struct iris_cmd_comp *pcmd_comp, uint8_t path)
{
	...
	if (path == PATH_DSI)
		...
		ret = _iris_i2c_s_send_ocp_cmds(panel, pcmd_comp);
	else
		ret = -EINVAL;
	IRIS_ATRACE_END("_iris_send_cmds");
...
int32_t iris_send_ipopt_cmds_i2c(int32_t ip, int32_t opt_id)
{
	...
	return _iris_send_cmds(pcfg->panel, &cmd_comp, PATH_I2C_S);
}
		...
		bool is_update, uint8_t path)
{
	...
	IRIS_LOGD("%s(), i_p: %#x opt: %#x, skip last: %d, update: %s",
		...
		struct iris_ctrl_opt *pip_opt, struct iris_cmd_comp *pcomp, uint8_t path)
{
	...
	IRIS_LOGD("%s(), i_p: %#x opt: %#x, skip last: %d.",
...
static void _iris_pre_timing_config(struct dsi_panel *panel)
{
	int len = 0;
	struct iris_update_ipopt popt[IP_OPT_MAX];
	bool use_2nd_timing = iris_belongs_to_2nd_timing(&panel->cur_mode->timing);
...
static void _iris_pre_rx_cmd(struct dsi_panel *panel)
{
	bool use_2nd_timing = iris_belongs_to_2nd_timing(&panel->cur_mode->timing);
...
{
	_iris_pre_timing_config(panel);
	_iris_pre_rx_cmd(panel);
...
int iris_lightup(struct dsi_panel *panel)
{
	...
	iris_update_frc_fps(panel->cur_mode->timing.refresh_rate & 0xFF);
...
int iris_enable(struct dsi_panel *panel, struct dsi_panel_cmd_set *on_cmds)
{
	...
	iris_update_cur_timing(&panel->cur_mode->timing);
...
exit_pt_lightup:
	pcfg->abypss_ctrl.abypass_mode =
			(prev_mode == ABP_SLEEP_MODE) ? ANALOG_BYPASS_MODE : PASS_THROUGH_MODE;
	...
	if (prev_mode != PASS_THROUGH_MODE)
		iris_abypass_switch_proc(pcfg->display, prev_mode, false, true);
		...
		struct dsi_panel_cmd_set *off_cmds)
{
	...
	IRIS_LOGI("%s(%d), mode: %s(%d) ---", __func__, __LINE__,
			iris_is_pt_mode(pcfg->panel) ? "PT" : "ABYP",
			pcfg->abypss_ctrl.abypass_mode);
	if (off_cmds) {
		if (iris_is_pt_mode(pcfg->panel))
...
static struct dsi_cmd_desc *_iris_get_desc_from_ipopt(uint8_t ip, uint8_t opt_id, int32_t pos)
{
	...
	if (popt == NULL) {
		IRIS_LOGE("%s(), can't find ip opt for i_p: 0x%02x, opt: 0x%02x.",
				__func__, ip, opt_id);
		return NULL;
	}
		...
		int32_t pos, uint32_t value)
{
	...
	for (cmd_list_index = IRIS_DTSI_PIP_IDX_START; cmd_list_index <= IRIS_DTSI_PIP_IDX_END; cmd_list_index++) {
		...
		if (popt == NULL) {
			IRIS_LOGW("%s(), can't find i_p: 0x%02x opt: 0x%02x, from cmd_list_index: %d",
		...
		pvalue[0] = value;
		IRIS_LOGD("sync cmd_list_index: %d,  i_p: %d, opt_id: %x, pos: %d, value: %x",
...
static int _iris_dev_probe(struct platform_device *pdev)
{
	...
	pcfg = iris_get_cfg();
	if (pcfg->panel == NULL) {
		IRIS_LOGW("%s(), iris doesn't init, defer probe", __func__);
		return -EPROBE_DEFER;
	}
