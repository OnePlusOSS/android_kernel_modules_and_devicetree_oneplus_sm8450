...
static int _iris_configure(u32 display, u32 type, u32 value)
{
	...
	case IRIS_S_CURVE:
		...
		if (pqlt_cur_setting->scurvelevel > 14)
			goto error;
	...
	case IRIS_CM_COLOR_GAMUT_PRE:
		pqlt_cur_setting->cm_sdr2hdr_enable = (value & 0x10) >> 4;
		iris_cm_color_gamut_pre_set(value & 0x03);
		break;
	case IRIS_CM_COLOR_GAMUT:
		pqlt_cur_setting->pq_setting.cmcolorgamut = value;
		if (pqlt_cur_setting->pq_setting.cmcolorgamut * 3 >= iris_get_lut_count(CM_LUT))
			goto error;
	...
	case IRIS_ANALOG_BYPASS_MODE:
		if (pcfg->abyp_ops.configure_abyp != NULL)
			pcfg->abyp_ops.configure_abyp(value);
		else
			IRIS_LOGI("pcfg->abyp_ops.configure_abyp is null!");
		break;
...
static int _iris_configure_ex(u32 display, u32 type, u32 count, u32 *values)
{
	...
	#endif
	u32 LutPos = 0;
	u32 dump_reg_cnt, dump_read_cnt, j;
	...
	case IRIS_DBG_TARGET_REG_DUMP:
		...
		for (i = 0; i < dump_read_cnt; i++) {
			for (j = 0; j < dump_reg_cnt; j++) {
				dump_reg_val[j] = dump_reg_addr[j];
				iris_i2c_ocp_read(&dump_reg_val[j], 1, 0);
			}
...
int iris_configure_get(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DBG_TARGET_REGADDR_VALUE_GET:
		...
		if (!iris_is_pt_mode(pcfg->panel) && (adb_type == 0))
			return -ENOTCONN;
