...
void iris_scaler_filter_ratio_get(void)
{
	...
	dwRatioDiff = pcfg->frc_setting.disp_hres * 10 / pcfg->frc_setting.mv_hres;
	...
	else if (dwScaleDownRatio < eScalerDown1_6)
		iris_scaler_filter_update(SCALER_PP, 0);
	else
	...
	if (iris_game_mode)
		iris_scaler_filter_update(SCALER_INPUT, 0);
	else
		iris_scaler_filter_update(SCALER_INPUT, (u32)dwScaleDownRatio);
	pcfg->add_last_flag = add_last_flag;
	IRIS_LOGD("add_last_flag: %d, add_on_last_flag: %d", pcfg->add_last_flag, pcfg->add_on_last_flag);
}
...
void iris_pq_parameter_init(void)
{
	...
	for (i = 0; i < 12; i++) {
		payload = iris_get_ipopt_payload_data(IRIS_IP_LCE, i, 2);
		m_LCE_gain[i][0] = (payload[0] & 0x3ff00000) >> 20;
		m_LCE_gain[i][1] = (payload[3] & 0xff000000) >> 24;
		m_LCE_gain[i][2] = payload[8] & 0x000003ff;
		m_LCE_gain[i][3] = (payload[8] & 0x000ffc00) >> 10;
	}
...
void iris_peaking_level_set(u32 level)
{
	...
	iris_capture_disable_pq(popt, &skiplast);
...
void iris_cm_para_level_set(u32 level)
{
	...
	iris_update_pq_opt(popt, len, path);
	iris_enable_ulps(path, is_ulps_enable);
	IRIS_LOGD("cm para level=%d, len=%d", level, len);
}
...
static int iris_cm_csc_para_set(struct iris_update_ipopt *popt, uint8_t skip_last, uint32_t csc_ip, uint32_t *csc_value)
{
	...
	if (psopt == NULL) {
		IRIS_LOGE("can not find i_p = %02x opt_id = %02x", ip, opt_id);
		return 1;
	}
...
void iris_cm_csc_level_set(u32 csc_i_p, u32 *csc_value)
{
	bool skiplast = 0;
	int len;
	struct iris_update_ipopt popt[IP_OPT_MAX];
	...
	iris_init_ipopt_ip(popt,  IP_OPT_MAX);
	iris_capture_disable_pq(popt, &skiplast);
	len = iris_cm_csc_para_set(popt, skiplast, csc_i_p, csc_value);
	len = iris_capture_enable_pq(popt, len);

	is_ulps_enable = iris_disable_ulps(path);
	iris_update_pq_opt(popt, len, path);
	iris_enable_ulps(path, is_ulps_enable);
	IRIS_LOGD("%s csc len=%d", (csc_i_p == IRIS_IP_DPP) ? "dpp" : "cm", len);
}

void iris_pq_adjust_set(u32 *csc_value)
{
	bool skiplast = 0;
	int len;
	struct iris_update_ipopt popt[IP_OPT_MAX];
	...
	iris_capture_disable_pq(popt, &skiplast);
  	...
  	for (i = 5; i < 8; i++) {
		if (csc_value[i] > 0x80000) {
			csc_value[i] = 0x100000 - csc_value[i];
			csc_value[i] = (csc_value[i] * 9)/10;
			csc_value[i] = 0x100000 - csc_value[i];
		} else {
			csc_value[i] = (csc_value[i] * 9)/10;
		}
	...
	psopt = iris_find_ip_opt(IRIS_IP_CM, 0x40);
	...
	data[7] = val;
	for (i = 0; i < 3; i++)
		data[8 + i] = csc_value[5 + i];

	for (i = 0; i < 8; i++)
		iris_set_ipopt_payload_data(IRIS_IP_CM, 0x40, 3 + i, data[3+i]);

  	psopt = iris_find_ip_opt(IRIS_IP_CM, 0x48);
	for (i = 0; i < 4; i++) {
		cm_csc_buffer[2 * i] = csc_value[i] & 0x7fff;
		cm_csc_buffer[2 * i + 1] = (csc_value[i] >> 16) & 0x7fff;
	}
	cm_csc_buffer[8] = csc_value[4] & 0x7fff;
	for (i = 0; i < 9; i++) {
		if (cm_csc_buffer[i] > 0x4000) {
			cm_csc_buffer[i] = 0x8000 - cm_csc_buffer[i];
			cm_csc_buffer[i] = (cm_csc_buffer[i] * 9)/10;
			cm_csc_buffer[i] = 0x8000 - cm_csc_buffer[i];
		} else {
			cm_csc_buffer[i] = (cm_csc_buffer[i] * 9)/10;
		}
	...
	for (i = 0; i < 4; i++)
		csc_value[i] = cm_csc_buffer[2 * i] + (cm_csc_buffer[2 * i + 1] << 16);
	csc_value[4] = cm_csc_buffer[8];
	...
	for (i = 0; i < 8; i++)
		iris_set_ipopt_payload_data(IRIS_IP_CM, 0x48, 3 + i, data[3+i]);

	if (!pcfg->dual_setting)
		cm_csc += 0x08;

	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_CM,
			cm_csc, skiplast);
...
int iris_cmlut_ratio_step_set(struct iris_update_ipopt *popt, uint8_t skiplast)
{
	u32 ratio;
	u32 regvalue = 0;
	...
	struct quality_setting *pqlt_cur_setting = &iris_setting.quality_cur;
	int len;
	u32 max_step = pqlt_cur_setting->cmlut_max_step;

	if (pqlt_cur_setting->cm_sdr2hdr_enable == 0)
		return 0;

	if (pqlt_cur_setting->cmlut_step == 0) {
		regvalue = pqlt_cur_setting->cmlut_type | 0x02;
	} else {
		ratio = ((max_step - pqlt_cur_setting->cmlut_step) * 16383) / max_step;
		regvalue = pqlt_cur_setting->cmlut_type | (ratio << 16);
	}
	...
	len = iris_init_update_ipopt_t(
			popt, IP_OPT_MAX, IRIS_IP_DPP, 0xfd, 0xfd, skiplast);
...
int iris_cm_ratio_set(struct iris_update_ipopt *popt, uint8_t skip_last)
{
	...
	if (pqlt_cur_setting->cm_sdr2hdr_enable)
		return 0;

	if (pqlt_cur_setting->pq_setting.cmcolortempmode == IRIS_COLOR_TEMP_MANUL_PLUS ||
			pqlt_cur_setting->pq_setting.cmcolortempmode == IRIS_COLOR_TEMP_AUTO_PLUS) {
		regvalue = pqlt_cur_setting->colortempplusvalue;
	} else {
...
void iris_cm_colortemp_mode_set(u32 mode)
{
	...
	if (pqlt_cur_setting->source_switch != 1) {
		...
		if (mode > IRIS_COLOR_TEMP_OFF) {
			if (pqlt_cur_setting->cm_sdr2hdr_enable == 0)
				iris_cm_ratio_set(popt, 0x01);
			else
				iris_cmlut_ratio_step_set(popt, 0x01);
		}
...
void iris_cm_ratio_step_set(u32 type, u32 step, u32 max_step)
{
	bool skiplast = 0;
	int len;
	struct iris_update_ipopt popt[IP_OPT_MAX];
	...
	iris_init_ipopt_ip(popt,  IP_OPT_MAX);
	iris_capture_disable_pq(popt, &skiplast);
	len = iris_cmlut_ratio_step_set(popt, skiplast);
	len = iris_capture_enable_pq(popt, len);
	is_ulps_enable = iris_disable_ulps(path);
	iris_update_pq_opt(popt, len, path);
	iris_enable_ulps(path, is_ulps_enable);
...
void iris_lce_gain_reset(bool enable, bool update)
{
	u32 locallevel;
	struct quality_setting *pqlt_cur_setting = &iris_setting.quality_cur;
	bool skiplast = 0;
	...
	payload = iris_get_ipopt_payload_data(IRIS_IP_LCE, locallevel, 2);

	payload[0] &= ~0x3ff00000;
	payload[3] &= ~0xff000000;
	payload[8] = 0;

	if (enable == false) {
		payload[0] |=  m_LCE_gain[locallevel][0] << 20;
		payload[3] |=  m_LCE_gain[locallevel][1] << 24;
		payload[8] = m_LCE_gain[locallevel][2] + (m_LCE_gain[locallevel][3] << 10);
	}
	...
	iris_init_ipopt_ip(popt,  IP_OPT_MAX);
	iris_capture_disable_lce(popt, &skiplast);
	...
	IRIS_LOGI("lce gain reset=%d, len=%d", enable, len);
}
...
void iris_lce_mode_set(u32 mode)
{
	...
	iris_lce_gain_reset(pqlt_cur_setting->lce_gain_reset, false);

	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_LCE, locallevel, 0x01);
...
void iris_lce_demo_window_set(u32 vsize, u32 hsize, u8 inwnd)
{
	...
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_LCE, 0xF0, 0x01);
	...
	if (pqlt_cur_setting->pq_setting.alenable == true) {
		...
		if (pqlt_cur_setting->pq_setting.sdr2hdr >= HDR10In_ICtCp &&
			...
			len = iris_update_ip_opt(popt, IP_OPT_MAX, AMBINET_SDR2HDR_LUT,
					0x0, skiplast);
			iris_get_ipopt_payload_data(IRIS_IP_SDR2HDR, (AL_SDR2HDR_INDEX_OFFSET + 2), 83);
			...
			len = iris_update_ip_opt(popt, IP_OPT_MAX, AMBINET_SDR2HDR_LUT,
					0x0, skiplast);
			iris_get_ipopt_payload_data(IRIS_IP_SDR2HDR, (AL_SDR2HDR_INDEX_OFFSET + 5), 83);
		}
	} else {
		u32 hdr_lut_idx;
...
void iris_ms_pwil_dma_update(struct iris_mspwil_parameter *par)
{
	...
	if (par->mvc_01phase_update) {
		...
		temp = (temp & (~0x00000004)) | (par->gmd_fmd_en<<2);
...
void iris_dtg_frame_rate_set(u32 framerate)
{
	...
	uint8_t path = iris_pq_update_path;
	struct iris_cfg *pcfg = iris_get_cfg();
	bool use_2nd_tm = false;

	iris_init_ipopt_ip(popt, IP_OPT_MAX);
	iris_capture_disable_pq(popt, &skiplast);

	use_2nd_tm = iris_belongs_to_2nd_timing(&pcfg->panel->cur_mode->timing);
	if (use_2nd_tm)
		iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DTG, 0x1, 0x0);
	else
		iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DTG, 0x0, 0x0);
...
void iris_pwil_sdr2hdr_resolution_set(bool enter_frc_mode)
{
	...
	if (pqlt_cur_setting->pq_setting.sdr2hdr >= SDR709_2_p3 && pqlt_cur_setting->pq_setting.sdr2hdr <= SDR709_2_2020) {
		...
		iris_init_ipopt_ip(popt,  IP_OPT_MAX);
		if (enter_frc_mode) {
			num_ratio = (1<<28) / (pcfg->frc_setting.mv_hres * pcfg->frc_setting.mv_vres);
			resolution = (pcfg->frc_setting.mv_vres & 0xFFF) | (pcfg->frc_setting.mv_hres & 0xFFF)<<16;
		} else {
		...
		iris_get_ipopt_payload_data(IRIS_IP_SDR2HDR, 0x93, 2);
...
void iris_dom_set(int mode)
{
	...
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DPORT, 0xF0, skiplast);
...
void iris_brightness_level_set(u32 *value)
{
	...
	iris_init_ipopt_ip(popt,  IP_OPT_MAX);
	len = iris_capture_disable_pq(popt, &skiplast);
	iris_brightness_para_set(popt, skiplast, value);
	len = iris_capture_enable_pq(popt, len);
	...
	if (pcfg && pcfg->osd_enable && pcfg->dynamic_vfr && pcfg->display->panel->is_secondary){
		IRIS_LOGV("clean video_update_wo_osd when update brightness in dual frc mode");
		iris_dynamic_vfr(pcfg,pcfg->display);
	}
}
		...
		struct iris_cfg *pcfg)
{
	...
	if (IRIS_IF_LOGV()) {
		IRIS_LOGE("i_p_cnt = %0x", pinit_val->ip_cnt);
		for (i = 0; i < pinit_val->ip_cnt; i++) {
			char ptr[256];
			int32_t len = 0;
			int32_t sum = 256;
...
void iris_cm_setting_switch(bool dual)
{
	...
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PWIL, pwil_csc, skiplast);
...
void iris_pwil_dpp_en(bool dpp_en)
{
	...
	if (dpp_en)
		...
		pwil_datapath &= ~0x40;
	if (dpp_en) // csc_mode change to auto when dpp disable
		pwil_datapath |= 0x10000;
	else
		pwil_datapath &= ~0x18000;
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xF1, 4, pwil_datapath);
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PWIL, 0xF1, 0x01);

	if (dpp_en) {	// force lut gen
		payload = iris_get_ipopt_payload_data(IRIS_IP_DPP, 0xfc, 2);
		cm_cntl_2 = payload[0];
		cmd[idx++] = 0xf1580040;
		cmd[idx++] = cm_cntl_2 & ~0x10;
		cmd[idx++] = 0xf1580040;
		cmd[idx++] = cm_cntl_2 | 0x10;
...
void iris_gamut_valid_set(bool valid)
{
	iris_gamut_valid = valid;
}

bool iris_gamut_valid_get(void)
{
	return iris_gamut_valid;
}
