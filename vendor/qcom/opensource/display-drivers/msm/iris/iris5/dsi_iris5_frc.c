...
void iris_frc_app_info(u32 info)
{
	struct iris_cfg *pcfg = iris_get_cfg();

	pcfg->frc_low_latency = 0;
	pcfg->frc_app_info = info;

	IRIS_LOGI("%s(%d) low latency: %d.", __func__, __LINE__, pcfg->frc_low_latency);
}
	...
	if (pcfg->rx_mode == DSI_OP_VIDEO_MODE && pcfg->tx_mode == DSI_OP_VIDEO_MODE) {
		...
		if (pcfg->abypss_ctrl.abypass_mode != PASS_THROUGH_MODE) {
			if (new_te != pcfg->panel_te) {
				...
				pcfg->ap_te = new_te;
				iris_update_frc_fps(new_te);
				IRIS_LOGI("%s, new panel TE: %d", __func__, new_te);
			}
		} else {
			// PT mode
			if (pcfg->pwil_mode == PT_MODE) {
				...
				if (new_te != pcfg->ap_te) {
					pcfg->ap_te = new_te;
					iris_dtg_setting_update(true);
					IRIS_LOGI("%s, new AP TE: %d", __func__, new_te);
				}
			}
		}
		return;
...
int32_t iris_parse_frc_setting(struct device_node *np, struct iris_cfg *pcfg)
{
	...
	pcfg->default_panel_te = pcfg->panel_te;
	pcfg->ap_te = 60;
...
int32_t iris_set_second_channel_power(bool pwr)
{
	...
	if (pwr) {	//on
		...
		if (pcfg->panel2->power_info.refcount == 0) {
			IRIS_LOGW("%s(), AP mipi2 tx hasn't been power on.", __func__);
			pcfg->osd_switch_on_pending = true;
		} else {
			if (pcfg->panel2->cur_mode && pcfg->panel2->cur_mode->priv_info &&
			    pcfg->panel2->cur_mode->priv_info->dsc_enabled)
		...
		if (!pcfg->osd_layer_empty && (pcfg->pwil_mode == FRC_MODE))
			iris_firep_force(false);
...
static void _iris_osd_blending_off(void)
{
	...
	//iris_set_pwil_disp_ctrl(); // update disp-ctrl directly
...
static void iris_frc_setting_switch(bool dual)
{
	...
	i = dual ? 1 : 0;
	pcfg->frc_setting.mv_hres = process_hres[i];
	pcfg->frc_setting.mv_vres = process_vres[i];
