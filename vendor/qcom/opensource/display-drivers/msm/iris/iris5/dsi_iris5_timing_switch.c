...
#ifdef IRIS_HDK_DEV // not port to customer
uint32_t iris_generate_switch_case(struct dsi_panel *panel,
		const struct dsi_mode_info *new_timing)
{
	return _iris_generate_switch_case(panel, new_timing);
}
#endif
...
void iris_sync_bitmask(struct iris_update_regval *pregval)
{
	...
	for (type = IRIS_DTSI_PIP_IDX_START; type <= IRIS_DTSI_PIP_IDX_END; type++) {
		...
		if (popt == NULL) {
			IRIS_LOGW("%s(), can't find i_p: 0x%02x opt: 0x%02x, from type: %d",
					__func__, ip, opt_id, type);
			continue;
		} else if (popt->cmd_cnt != 1) {
			IRIS_LOGW("%s(), invalid bitmask for i_p: 0x%02x, opt: 0x%02x, type: %d, popt len: %d",
		...
		struct dsi_mode_info *new_timing)
{
	...
	if (switch_case == SWITCH_PT_TO_ABYP) {
		iris_abypass_switch_proc(to_dsi_display(panel->host), ANALOG_BYPASS_MODE, false, true);
		rc = iris_dsi_send_cmds(panel, switch_cmds->cmds, switch_cmds->count, switch_cmds->state);
	}
