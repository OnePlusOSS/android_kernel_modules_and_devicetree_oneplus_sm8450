...
static int iris_i2c_cmd_four_read(struct addr_val *val, int len)
{
	...
	twelve_data_list = kmalloc(12 * len * sizeof(uint8_t), GFP_KERNEL);
	if (!twelve_data_list)
		return -ENOMEM;

	msgs = kmalloc(2 * len * sizeof(struct i2c_msg), GFP_KERNEL);
	if (!msgs)
		goto FREE_BUFFER;

	slave_addr = (client->addr & 0xff);
	memset(msgs, 0x00, 2 * len * sizeof(struct i2c_msg));
...
static int iris_i2c_cmd_four_write(struct addr_val *val, int len)
{
	...
	msgs = kmalloc(len * sizeof(struct i2c_msg), GFP_KERNEL);
	if (!msgs)
		goto FREE_BUFFER;

	slave_addr = (client->addr & 0xff);
	memset(msgs, 0x00, len * sizeof(struct i2c_msg));
	byte_count = 8 + 4;
...
static int iris_i2c_ocp_single_read(uint32_t *ptr, uint32_t len)
{
	...
	p = kmalloc(sizeof(struct addr_val) * (len+1), GFP_KERNEL);
	if (p == NULL) {
...
static int iris_i2c_ocp_burst_read(uint32_t *ptr, uint32_t dlen)
{
	...
	iris_payload = kmalloc(sizeof(uint8_t) * (8+msg_len), GFP_KERNEL);
...
int iris_i2c_ocp_burst_write(uint32_t *arr, uint32_t dlen)
{
	...
	iris_payload = kmalloc(sizeof(uint8_t) * msg_len + 1, GFP_KERNEL);
	if (iris_payload == NULL) {
		pr_err("[iris] %s: allocate memory fails\n", __func__);
		return -EINVAL;
	}
...
int iris_i2c_ocp_single_write(uint32_t *arr, uint32_t dlen)
{
	...
	p = kmalloc(sizeof(struct addr_val) * (tlen+1), GFP_KERNEL);
	if (p == NULL) {
		pr_err("[iris] %s: allocate memory fails\n", __func__);
		return -EINVAL;
	}
