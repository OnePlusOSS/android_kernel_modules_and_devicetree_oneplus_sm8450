		...
		struct dsi_cmd_desc *dsi_cmd, struct dsi_panel_cmd_set *cmdset)
{
	...
	for (i = 0; i < sum; i++) {
		...
		flags = (i == sum - 1) ? dsi_cmd->msg.flags :
				(dsi_cmd->msg.flags | MIPI_DSI_MSG_BATCH_COMMAND);
...
static int _iris_pt_read(struct dsi_panel_cmd_set *cmdset)
{
	u32 i = 0;
	u32 rlen = 0;
	u32 intstat = 0;
