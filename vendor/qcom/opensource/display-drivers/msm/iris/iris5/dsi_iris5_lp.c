...
static int _iris_dphy_itf_check(bool aux_channel);
...
void iris_init_abyp_ops(void)
{
	...
	case IRIS_CHIP_VER_1:
		...
		if (pcfg->panel->panel_mode == DSI_OP_CMD_MODE)
...
void _iris_video_abyp_power(bool on)
{
	...
	IRIS_LOGI("%s 0x%x 0x%x", __func__, regval.mask, regval.value);

	iris_update_bitmask_regval_nonread(&regval, false);

	if (pcfg->iris_i2c_switch)
		iris_send_ipopt_cmds_i2c(IRIS_IP_SYS, regval.opt_id);
	else
		iris_send_ipopt_cmds(IRIS_IP_SYS, regval.opt_id);
}
...
static int _iris_dphy_itf_check(bool aux_channel)
{
	int i = 0;
	int ret = 0;
	u32 dphy_itf_ctrl = MIPI_MAIN_DPHY_ITF_CTRL;
	u32 arr[2] = {0};
	u32 val = 0;

	if (aux_channel)
		dphy_itf_ctrl = MIPI_AUX_DPHY_ITF_CTRL;
	while (i < 10) {
		arr[0] = dphy_itf_ctrl;
		ret = iris_i2c_conver_ocp_read(arr, 1, false);
		if (ret) {
			IRIS_LOGE("%s, i2c read [0x%x] fail, ret = %d", __func__,
				dphy_itf_ctrl, ret);
			return -EINVAL;
		}
		...
		if (val & 0x10) {
			if (val & 0x10000000) {
			IRIS_LOGI("%s, ST_DPHY_DSI_FLUSH is 1, [0x%x] = 0x%08x", __func__,
				dphy_itf_ctrl, val);
				arr[0] = dphy_itf_ctrl;
	...
	if (i == 10) {
		ret = -EINVAL;
		IRIS_LOGE("%s [0x%x] failed for %d time", __func__, dphy_itf_ctrl, i);
	}
	return ret;
}
...
static void _iris_post_config_for_timing(void)
{
	...
	iris_update_frc_fps(pcfg->panel->cur_mode->timing.refresh_rate & 0xFF);

	pcfg->abypss_ctrl.pending_mode = MAX_MODE;
	pcfg->abypss_ctrl.abypass_mode = PASS_THROUGH_MODE;

	if (pcfg->panel->qsync_mode > 0)
		iris_qsync_set(true);
}
...
static void _iris_video_abyp_exit(void)
{
	...
	usleep_range(1000 * 5, 1000 * 5 + 1);

	/*configure other domains IP via DMA trigger */
	if (pcfg->iris_i2c_switch)
		iris_send_ipopt_cmds_i2c(IRIS_IP_DMA, 0xe5);
	else
		iris_send_ipopt_cmds(IRIS_IP_DMA, 0xe5);
...
bool iris_abypass_switch_proc(struct dsi_display *display, int mode, bool pending, bool first)
{
	...
	struct iris_cfg *pcfg = iris_get_cfg();
	int prev_mode = pcfg->abypss_ctrl.abypass_mode;
	bool pt_mode = iris_is_pt_mode(pcfg->panel);
...
{
	//To do: need to fix, moved from iris7
	return true;
}
...
void iris_qsync_set(bool enable)
{
	...
	if (iris_is_pt_mode(pcfg->panel)) {
		use_2nd_timing = iris_belongs_to_2nd_timing(&pcfg->panel->cur_mode->timing);

		ip = IRIS_IP_DTG;
		opt_id = use_2nd_timing ? 0x01 : 0x00;
		psopt = iris_find_ip_opt(ip, opt_id);
		if (psopt == NULL) {
			IRIS_LOGE("%s, can not find i_p = %02x opt_id = %02x", __func__, ip, opt_id);
			return;
		}
...
void iris_set_metadata(bool panel_lock)
{
	...
	IRIS_LOGI("dport_meta: %x, osd_meta: %x, dpp_meta: %x, lock",
			dport_meta, osd_meta, dpp_meta, panel_lock);
	pcfg->metadata = 0;
...
int iris_status_get(struct dsi_display_ctrl *ctrl, struct dsi_panel *panel)
{
	...
	// check abyp mode
	mode = pcfg->abypss_ctrl.abypass_mode;
								 ...
								 size_t count, loff_t *ppos)
{
	...
	tot += scnprintf(bp + tot, buf_len - tot,
					 "chip_id: %d\n", pcfg->chip_id);
	tot += scnprintf(bp + tot, buf_len - tot,
					 "panel mode: %s\n", (pcfg->panel->panel_mode == DSI_OP_CMD_MODE)?"cmd":"video");
	tot += scnprintf(bp + tot, buf_len - tot,
					 "abyp enable: %d\n",pcfg->lp_ctrl.abyp_enable);
	tot += scnprintf(bp + tot, buf_len - tot,
					 "abyp default mode: %d\n",pcfg->abypss_ctrl.default_abyp_mode);
	if (copy_to_user(buff, bp, tot))
		return -EFAULT;
	*ppos += tot;
