...
static int _iris_send_lut_for_dma(void)
{
	...
	if (rc)
		IRIS_LOGE("%s(%d), failed to change sdr2hdr type", __func__, __LINE__);
...
{
	int ret = 0;
	struct iris_data data[] = { {NULL, 0}, {NULL, 0}, {NULL, 0} };
...
load_ccf_only:
	...
	if (!ret) {
		...
		if ((ccf2_fw->size - ccf2_tail_size) % ccf2_per_pkg_size == 0
			...
			fw_ccf_status |= fw_calibrate_st<<8;
	...
	fw_loaded_status = (firmware_state == 0x07 ? FIRMWARE_LOAD_SUCCESS : FIRMWARE_LOAD_FAIL);
	mutex_unlock(&fw_status_lock);
...
int iris_send_lut(u8 lut_type, u8 lut_table_index, u32 lut_abtable_index)
{
	...
	case SCALER1D_PP_LUT:
		...
		lut_opt_id = lut_table_index & 0xff;
		_iris_fomat_lut_cmds(lut_type, lut_opt_id);
		IRIS_LOGI("%s(), call SCALER1D, i_p: %#x, index: %d.", __func__,
