...
/* Solve the problem that get swr-dev fails after adsp ssr */
...
static int wcd938x_get_logical_addr(struct swr_device *swr_dev)
{
	...
	do {
		#ifdef OPLUS_ARCH_EXTENDS
		/*add CR#3252303 to fix swr issue*/
		/* retry after 4ms */
		...
		if (ret) {
			dev_info(&swr_dev->dev, "%s get devnum %d for dev addr %llx failed, ret = %d, num_retry = %d",
