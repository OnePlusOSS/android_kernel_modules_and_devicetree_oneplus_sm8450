...
/*
	...
	{0xF088, 0x0000, 0x0001, 0x00},  // OIS Gyro Offset Req
	...
	*/
	...
	{0xF088, 0x02,   0x0001, 0x01},  // OIS gyro raw data
	{0xF08A, 0x0000, 0x0002, 0x00},  // OIS gyro raw data X
	{0xF088, 0x03,   0x0001, 0x01},  // OIS gyro raw data
	{0xF08A, 0x0000, 0x0002, 0x00},  // OIS gyro raw data 
};
					...
					if (ois_registers_bu24721[i][0]) {
						...
						if(0x00 == ois_registers_bu24721[i][3])
						{
							...
							rc = camera_io_dev_read(&(o_ctrl->io_master_info), (uint32_t)ois_registers_bu24721[i][0], (uint32_t *)&ois_registers_bu24721[i][1],
							                   CAMERA_SENSOR_I2C_TYPE_WORD, ois_reg_type);
						}
...
int RohmOisWrite(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr, uint32_t data)
{
	...
	for(i = 0; i < retry; i++) {
        ...
        CAM_DBG(CAM_OIS, "write cci device=%d master=%d",o_ctrl->io_master_info.cci_client->cci_device,o_ctrl->io_master_info.cci_client->cci_i2c_master);
            CAM_DBG(CAM_OIS, "write success ois type=%d,write 0x%04x ,data=%d",o_ctrl->ois_type, addr,data);
			return rc;
		}
	}
	return rc;
...
int DownloadFW(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl)
	{
		...
		if (strstr(o_ctrl->ois_name, "ov64b_ois_sem1217s"))
		{
			CAM_ERR(CAM_OIS, "sem1217s firmware download");
			ois_ctrl = o_ctrl;
...
int OISPollThreadBu24721(void *arg)
{
	struct cam_ois_ctrl_t *o_ctrl                   = (struct cam_ois_ctrl_t *)arg;
	int32_t                i;
	int32_t                rc;
	...
	CAM_ERR(CAM_OIS, "OISPollThreadBu24721 creat");
	while(1) {
		rc          = 0;
		offset      = 0;
		...
		rc = camera_io_dev_read_seq(&(o_ctrl->io_master_info), 0xF200, data,
		CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, OIS_HALL_DATA_SIZE);
		mutex_unlock(&(o_ctrl->ois_poll_thread_mutex));
		...
		if (dataNum == HALL_MAX_NUMBER || isFirstFrame) {
			...
			isFirstFrame = false;
			CAM_ERR(CAM_OIS,"Data drop!!,reset LastPacketEndTime: sec = %d, us = %d",LastPacketEndTime.tv_sec,LastPacketEndTime.tv_usec);
		}
		...
		if ((kfifo_len(&(o_ctrl->ois_hall_data_fifo)) + validSize) <= fifo_size_in_driver) {
			...
			if (kfifo_in_len != validSize) {
				CAM_ERR(CAM_OIS, "kfifo_in %d Bytes, FIFO maybe full, some OIS Hall sample maybe dropped.", kfifo_in_len);
			} else {
		...
		if ((kfifo_len(&(o_ctrl->ois_hall_data_fifoV2)) + validSize) <= fifo_size_in_driver) {
			...
			if (kfifo_in_len != validSize) {
				CAM_ERR(CAM_OIS, "ois type=%d,kfifo_in %d Bytes, FIFOV2 maybe full, some OIS Hall sample maybe dropped.",o_ctrl->ois_type, kfifo_in_len);
			} else {
...
exit:
	...
	CAM_ERR(CAM_OIS, "OISPollThreadBu24721 exit");
	return 0;
}
...
int OIS_READ_HALL_DATA_TO_UMD_SEM1217S (struct cam_ois_ctrl_t *o_ctrl,struct i2c_settings_array *i2c_settings)
{
	...
	list_for_each_entry(i2c_list,
		...
		CAM_DBG(CAM_OIS,"read done fifo_count %x %x%x, %x%x qtimer %lld ns_1 %lld laps %lld",
			fifo_count, temp_buff[0], temp_buff[1], temp_buff[2], temp_buff[3], qtimer_ns, qtimer_ns_1, (qtimer_ns-qtimer_ns_1));
#if 0
		{
			uint32_t shiftX = 0;
			uint32_t shiftY = 0;
			rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0x1104, &shiftX, CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_WORD);
			if (rc < 0)
			{
				CAM_ERR(CAM_OIS, "ois type=%d,Continue read failed, rc:%d, retry:%d",o_ctrl->ois_type, rc, 0);
			}
			rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0x1106, &shiftY, CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_WORD);
			if (rc < 0)
			{
				CAM_ERR(CAM_OIS, "ois type=%d,Continue read failed, rc:%d, retry:%d",o_ctrl->ois_type, rc, 0);
			}
			CAM_DBG(CAM_OIS,"read done shiftX 0x%x, shiftY 0x%x", shiftX, shiftY);
		}
#endif
		...
		delayCount = 0; // ((uint32_t)(temp_buff[j + 1] << 8) | temp_buff[j + 2]);
		...
		if(fifo_count > 0) {
			for(i=0 ; i<fifo_count; i++){
				hall_data_x=((read_buff[i*4]<<8)+read_buff[i*4+1]);
				hall_data_y=((read_buff[i*4+2]<<8)+read_buff[i*4+3]);
				CAM_DBG(CAM_OIS,"hall_data_x=%d hall_data_y=%d ",
					hall_data_x,
					hall_data_y);
			}
		}
...
void InitOISResource(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl)
	{
		...
		{
			CAM_ERR(CAM_OIS, "create ois download fw thread for sm1217");
...
int Rohm_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	{
		//1.download Fw
		CAM_ERR(CAM_OIS, "Tele OIS download FW ...");
