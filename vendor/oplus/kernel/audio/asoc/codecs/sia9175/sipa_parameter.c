...
#else
static const char *sipa_fw_path_w = "/odm/firmware";
#endif
	...
	void *context)
{
	...
	/* load writeable fw */
/* kernel, only read. jennie 2022/03/25/ */
#if 0
...
#if 0
	if (param_w->crc != crc32((uint8_t *)&param_w->version, sizeof(SIPA_PARAM_WRITEABLE) - sizeof(uint32_t))) {
	...
	int32_t a)
{
	...
	file = filp_open_block(fw_path, O_WRONLY, 0);
	if (IS_ERR(file)) {
	...
	ret = file->f_op->write(file, (const char *)&sipa_parameters->writeable.cal_spk[ch],
	...
	SIPA_PARAM_SPK_MODEL_PARAM *spk_model)
{
	...
	ret = file->f_op->write(file, (const char *)&sipa_parameters->writeable.spk_model[ch],
