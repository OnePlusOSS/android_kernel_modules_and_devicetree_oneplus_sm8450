...
static DEFINE_RWLOCK(omrg_list_lock);
...
static struct omrg_freq_device *omrg_get_cpu_dev(unsigned int cpu)
{
	...
	read_lock(&omrg_list_lock);
	/* find the freq device of cpu */
	list_for_each_entry(freq_dev, &g_omrg_cpufreq_list, node) {
		if (cpumask_test_cpu(cpu, &freq_dev->cpus)) {
			read_unlock(&omrg_list_lock);
			return freq_dev;
		}
	}
...
static struct omrg_freq_device *omrg_get_devfreq_dev(struct devfreq *df)
{
	...
	list_for_each_entry(freq_dev, &g_omrg_devfreq_list, node) {
		if (freq_dev->df == df) {
			read_unlock(&omrg_list_lock);
			return freq_dev;
		}
				     ...
				     unsigned int target_freq)
{
	...
	return omrg_freq_check_limit(freq_dev, target_freq);
}
...
static void omrg_freq_dev_release_work(struct kthread_work *work)
{
	struct omrg_device *omrg_dev = NULL;
	struct omrg_device *tmp = NULL;
	struct omrg_freq_device *freq_dev =
			container_of(work, struct omrg_freq_device, destroy_work);
...
static void omrg_destroy_freq_dev(struct omrg_freq_device *freq_dev)
{
	if (g_omrg_initialized) {
		kthread_init_work(&freq_dev->destroy_work,
				  omrg_freq_dev_release_work);
		kthread_queue_work(&g_omrg_worker, &freq_dev->destroy_work);
...
void omrg_cpufreq_register(struct cpufreq_policy *policy)
{
	...
	list_add_tail(&freq_dev->node, &g_omrg_cpufreq_list);
	if (g_omrg_initialized)
		of_match_omrg_rule(freq_dev);

	write_unlock(&omrg_list_lock);
...
void omrg_cpufreq_unregister(struct cpufreq_policy *policy)
{
	...
	if (found) {
		omrg_freq_dev_release_work(&freq_dev->destroy_work);
...
void omrg_devfreq_register(struct devfreq *df)
{
	...
	list_add_tail(&freq_dev->node, &g_omrg_devfreq_list);
	if (g_omrg_initialized)
		of_match_omrg_rule(freq_dev);
unlock:
	write_unlock(&omrg_list_lock);
...
void omrg_devfreq_unregister(struct devfreq *df)
{
	...
	if (found)
		omrg_destroy_freq_dev(freq_dev);
}
