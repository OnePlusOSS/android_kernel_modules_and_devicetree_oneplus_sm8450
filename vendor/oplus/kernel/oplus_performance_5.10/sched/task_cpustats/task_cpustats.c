...
static int task_cpustats_enable = 0;
...
static struct proc_dir_entry *task_cpustats = NULL;
...
static int proc_task_cpustats_init(void)
{
	...
	proc_entry = proc_create("task_cpustats_enable", S_IRUGO | S_IWUGO, task_cpustats, &task_cpustats_enable_proc_fops);
	if (!proc_entry) {
		task_err("create task_cpustats_enable failed\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("task_cpustats", S_IRUGO | S_IWUGO, NULL, &task_cpustats_proc_fops);
	if (!proc_entry) {
		task_err("create task_cpustats failed\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("sgeinfo", S_IRUGO | S_IWUGO, task_cpustats, &sge_proc_fops);
	if (!proc_entry) {
		task_err("create sgeinfo failed\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("sgefreqinfo", S_IRUGO | S_IWUGO, task_cpustats, &sgefreq_proc_fops);
	if (!proc_entry) {
...
static int unregister_vendor_hooks()
{
	int rc = 0;
