...
#include "../healthinfo/healthinfo.h"
#include "obinder_control.h"
...
module_param_named(oplus_binder_control_enable, sysctl_ob_control_enable, uint,
		   0660);
module_param_named(oplus_binder_status_record_enable,
		   sysctl_binder_status_record, uint, 0660);
/*
static void
...
#define REGISTER_TRACE_VH(vender_hook, handler) \
		 { \
			...
			if (ret) { \
				pr_info("failed to register_"#vender_hook", ret=%d\n", ret); \
				return ret; \
			} \
		}
...
static int register_scheduler_vendor_hooks(void)
{
	...
	/* register vender hook in drivers/android/binder.c */
	REGISTER_TRACE_VH(android_vh_binder_looper_state_registered,
			  android_vh_binder_looper_state_registered_handler);
	REGISTER_TRACE_VH(android_vh_binder_free_proc,
			  android_vh_binder_free_proc_handler);
	REGISTER_TRACE_VH(android_vh_binder_thread_release,
			  android_vh_binder_thread_release_handler);
	REGISTER_TRACE_VH(android_vh_binder_has_work_ilocked,
			  android_vh_binder_has_work_ilocked_handler);
	REGISTER_TRACE_VH(android_vh_binder_read_done,
			  android_vh_binder_read_done_handler);
	REGISTER_TRACE_VH(android_vh_binder_restore_priority,
			  android_vh_binder_restore_priority_handler);
	REGISTER_TRACE_VH(android_vh_binder_preset,
			  android_vh_binder_preset_handler);
	REGISTER_TRACE_VH(android_vh_binder_proc_transaction_entry,
			  android_vh_binder_proc_transaction_entry_handler);
	REGISTER_TRACE_VH(android_vh_binder_proc_transaction_finish,
			  android_vh_binder_proc_transaction_finish_handler);
	REGISTER_TRACE_VH(android_vh_binder_select_worklist_ilocked,
			  android_vh_binder_select_worklist_ilocked_handler);
				     ...
				     size_t count, loff_t *ppos)
{
	...
	len =
	    snprintf(buffer, sizeof(buffer), "obs enable=%d\n",
		     sysctl_ob_control_enable);
