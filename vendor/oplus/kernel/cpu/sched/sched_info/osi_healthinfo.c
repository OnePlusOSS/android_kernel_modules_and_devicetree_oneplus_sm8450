...
void ohm_para_init(void)
{
	...
	ohm_logon_init();
	ohm_trig_init();
	osi_debug("origin list: ctrl 0x%08x, logon 0x%08x, trig 0x%08x\n", ohm_ctrl_list, ohm_logon_list, ohm_trig_list);
	return;
}
...
/********** Legal Check **********/
			osi_err("Para illegal: sched_type %s, l_ms %d, h_ms %d\n",
	...
	osi_debug("Success update ohm_para!\n");
}
...
static ssize_t ohm_thresh_write_common(struct file *file, const char __user *buff, size_t len, loff_t *ppos, int type)
{
	...
	osi_debug("thresh update success!Now %s h_ms= %dms,l_ms= %dms\n",
		sched_list[type], ohm_thresh_para[type].h_ms, ohm_thresh_para[type].l_ms);
	return len;
...
static void healthinfo_alloc_pages_slowpath_begin(void *data, gfp_t gfp_mask, unsigned int order, unsigned long *alloc_start)
{
	*alloc_start = jiffies;
}

static void healthinfo_alloc_pages_slowpath_end(void *data, gfp_t gfp_mask, unsigned int order, unsigned long alloc_start)
{
...
static int register_healthinfo_vendor_hooks(void)
{
	...
	REGISTER_TRACE_VH(android_vh_alloc_pages_slowpath_begin, healthinfo_alloc_pages_slowpath_begin);
	REGISTER_TRACE_VH(android_vh_alloc_pages_slowpath_end, healthinfo_alloc_pages_slowpath_end);
	return ret;
}
...
static int unregister_healthinfo_vendor_hooks(void)
{
	...
	UNREGISTER_TRACE_VH(android_vh_alloc_pages_slowpath_begin, healthinfo_alloc_pages_slowpath_begin);
	UNREGISTER_TRACE_VH(android_vh_alloc_pages_slowpath_end, healthinfo_alloc_pages_slowpath_end);
	return ret;
}
