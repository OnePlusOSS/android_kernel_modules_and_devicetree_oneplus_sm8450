...
unlock_fbg:
	...
	*util = max(raw_util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug)) {
		cfs_policy_util_systrace_c(boosted_policy_util);
		if (policy_util)
			cfs_curr_util_systrace_c(grp->curr_util);
		else
			cfs_curr_util_systrace_c(0);
	}
	...
	*util = max(*util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug)) {
		rt_policy_util_systrace_c(boosted_policy_util);
		if (policy_util)
			rt_curr_util_systrace_c(grp->curr_util);
		else
			rt_curr_util_systrace_c(0);
		raw_util_systrace_c(raw_util);
		query_cpus_systrace_c(cpumask_first(query_cpus));
	}
