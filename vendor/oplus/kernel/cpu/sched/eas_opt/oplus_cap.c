...
unsigned long real_cpu_cap[OPLUS_CPUS] = {1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024};
...
noinline int oplus_tracing_mark_write(const char *buf)
{
	trace_printk(buf);
	return 0;
}
...
void oplus_cap_systrace_c(unsigned int cpu, unsigned long cap_orig, unsigned long cap)
{
	...
	oplus_tracing_mark_write(buf_orig);
	snprintf(buf, sizeof(buf), "C|10001|Cpu%d_real_cap_orig|%d\n", cpu, cap);
	oplus_tracing_mark_write(buf);
}
...
static int set_cluster_oplus_cap(int cluster_idx, int oplus_cap_multiple)
{
	...
	for_each_cpu(cpu, &cpus) {
		...
		rq = cpu_rq(cpu);
		rq->cpu_capacity_orig = mult_frac(thermal_cap, oplus_cap_multiple, 100);
...
static const struct proc_ops proc_oplus_cap_fops = {
	.proc_write		= proc_oplus_cap_write,
	.proc_read		= proc_oplus_cap_read,
};
...
int oplus_cap_init(struct proc_dir_entry *dir)
{
	...
	proc_node = proc_create("eas_opt_enable", 0666, oplus_cap_dir_parent, &proc_eas_opt_enabled_fops);
...
void oplus_cap_proc_remove(struct proc_dir_entry *dir)
{
	...
	remove_proc_entry("eas_opt_enable", oplus_cap_dir_parent);
