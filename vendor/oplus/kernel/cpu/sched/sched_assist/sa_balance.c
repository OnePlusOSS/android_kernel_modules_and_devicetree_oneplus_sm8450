...
static int task_hot(struct task_struct *p, struct lb_env *env)
{
	...
	/*
	 * Modefied by oplus.
	 * We need another way to judge whether this task
	 * is cfs because fair_sched_class is not exported.
 ...
 * its new rq.
 */
__maybe_unused static void attach_one_task(struct rq *rq, struct task_struct *p)
{
	struct rq_flags rf;
...
static struct task_struct *oplus_detach_running_task(struct lb_env *env)
{
	...
	list_for_each_entry(p,
		...
		/*
		 * Skip tasks that are being migrated to other CPUs.
		 */
		if (!task_on_rq_queued(p))
			continue;

		/*
		 * The task to be migrated must be in the TASK_RUNNING state.
		 */
		if (p->state != TASK_RUNNING)
			continue;

		/*
		 * Skip if this task is running on other cpu.
		 */
		if (task_cpu(p) != env->src_cpu)
			continue;

		/*
		 * Detach this task from src_cpu when all the above conditions
		 * are met.
		 */
		detach_task(p, env);
...
static struct task_struct *oplus_detach_running_task_for_rt(struct lb_env *env)
{
	...
	plist_for_each_entry(p, head, pushable_tasks) {
		...
		trace_printk("DEBUG_LB_RT_TICK[%d]: p=%s$%d, lb.pid=%d "
			...
			p->state, p->on_cpu, p->on_rq, task_cpu(p));
#endif
...
static int oplus_active_load_balance_cpu_stop_for_rt(void *data)
{
	...
	if (task) {
		trace_printk("DEBUG_LB_RT_TICK[%d]: task=%s$%d, state=%d, on_cpu=%d, on_rq=%d, task_cpu=%d\n",
			__LINE__, task->comm, task->pid,
			task->state, task->on_cpu, task->on_rq, task_cpu(task));
	}
#endif
...
static bool oplus_migrate_runnable_ux(void *data, struct rq *rq)
{
	 ...
	 */
	ux_task = oplus_pick_runnable_ux(this_cpu, -1, NULL, true);
	if (!ux_task)
		return false;
			...
			struct rq *rq, bool rt_boost)
{
	...
	trace_printk("DEBUG_LB_RT_TICK[%d]: curr=%s$%d, "
		...
		"rt_boost=%d, ret=%d\n",
		__LINE__, curr->comm, curr->pid,
		curr->state, curr->on_cpu, curr->on_rq, task_cpu(curr),
			...
			int order_idx, int walk_cnt)
{
	...
	for (idx = 0; idx < walk_cnt; idx++) {
		...
		for_each_cpu(cpu, &search_cpus) {
			 ...
			 */
			ux_task = oplus_pick_runnable_ux(cpu, this_cpu, NULL, false);
			if (!ux_task) {
				double_unlock_balance(this_rq, busiest_rq);
				continue;
			}
