...
void uag_update_counter(void *ptr)
{
	...
	for_each_cpu(cpu, pol->cpus) {
		...
		for (i = 0; i < SYS_AMU_MAX; ++i) {
			...
			per_cpu(amu_delta, cpu).val[i] = data.val[i] -
				per_cpu(amu_prev_cntr, cpu).val[i];
			per_cpu(amu_prev_cntr, cpu).val[i] = data.val[i];
		}
		per_cpu(amu_update_delta_time, cpu) = delta_time =
			ktime_sub(time, per_cpu(amu_last_update_time, cpu));
