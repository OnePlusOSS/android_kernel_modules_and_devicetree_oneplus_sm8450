...
static void soft_limit_set_freq(struct uag_gov_policy *sg_policy, unsigned long *target_util)
{
	...
	trace_soft_limit(soft_freq, raw_soft_util, soft_util,
			ua_util, raw_util, break_freq_margin, *target_util);
}
#endif
...
void update_util_tl(struct uag_gov_policy *sg_policy, unsigned long *util)
{
	...
	if (sg_policy && !capacity_margin_dvfs_changed) {
		...
		if (target_util >= 0)
			*util = target_util;
		trace_uag_next_util_tl(sg_policy->policy->cpu,
				*util, arch_scale_cpu_capacity(sg_policy->policy->cpu), target_util);
	} else {
				...
				struct cpumask *cpumask, unsigned long *next_freq)
{
	...
	for (i = 0; i < pd_count; i++) {
		...
		*next_freq = info->util_freq[util];
		...
		if (sg_policy)
			trace_uag_next_freq_tl(sg_policy->policy->cpu,
					raw_util, raw_freq, util, *next_freq);
#endif /* CONFIG_OPLUS_UAG_USE_TL */
		break;
	}
	...
	for (i = 0; i < pd_count; i++) {
		...
		for_each_cpu(j, &pd_info->cpus) {
			...
			if (per_cpu(cpu_scale, j) != pd_info->caps[pd_info->nr_caps - 1]) {
				pr_info("per_cpu(cpu_scale, %d)=%d,"
					"pd_info->caps[pd_info->nr_caps - 1]=%d\n",
