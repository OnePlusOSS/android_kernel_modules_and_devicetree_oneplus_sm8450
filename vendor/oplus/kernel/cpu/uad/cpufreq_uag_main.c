	...
	do {
		...
		if (util > prevutil) {
			...
			if (util <= utilmin) {
				 ...
				 * than freqmin.
				 */
				util = find_closest_util(sg_policy, utilmin + 1,
						CPUFREQ_RELATION_L);
				/*
				  ...
				  unsigned int freq)
{
	...
	if (freq == 0)
		...
		if (opp < 0) {
			pr_err("get opp idx failed! cpu = %d freq = %d",
					sg_policy->policy->cpu, freq);
			return 0;
		}
	}
	trace_printk("opp = %lu\n", opp);
				...
				unsigned long nl, unsigned long *util, unsigned long *max)
{
	...
	trace_printk("cpu_util = %lu avg_cap = %lu, hispeed_util = %lu,"
		"is_hiload = %d, is_migration = %d, nl = %lu, max = %lu, util = %lu\n",
		cpu_util, sg_policy->avg_cap, sg_policy->hispeed_util,
		is_hiload, is_migration, nl, *max, *util);
}
#else
static void uag_gov_util_adjust(struct uag_gov_cpu *sg_cpu, unsigned long cpu_util,
				unsigned long *util, unsigned long *max)
 ...
 */
static inline void ignore_dl_rate_limit(struct uag_gov_cpu *sg_cpu,
		struct uag_gov_policy *sg_policy)
{
...
static unsigned int uag_gov_get_final_freq(struct uag_gov_cpu *sg_cpu,
		struct uag_gov_policy *sg_policy, unsigned int next_freq)
{
				...
				unsigned int flags)
{
	...
	if (sg_policy->tunables->multi_tl_enable) {
		fbg_freq_policy_util(sg_cpu->sg_policy->flags,
				sg_policy->policy->cpus, &sg_policy->multi_util[UA_UTIL_FBG]);
...
#else
	/* Adjust &util with frame boost policy, avoiding frame missing */
	fbg_freq_policy_util(sg_cpu->sg_policy->flags, sg_policy->policy->cpus, &util);
#endif
#endif
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_VT_CAP)
...
#endif
		if (unlikely(eas_opt_debug_enable))
			trace_printk("[eas_opt]: cluster_id: %d, capacity: %d, util_thresh: %d, util_orig: %d, util: %d, avg_nr_running: %d, fake_cap_multiple: %d,nr_fake_cap_multiple: %d, util_thresh: %d\n",cluster_id, max, util_thresh, util_orig, util, avg_nr_running, fake_cap_multiple[cluster_id], nr_fake_cap_multiple[cluster_id], util_thresh_percent[cluster_id]);
	}
#endif
...
static unsigned int uag_gov_next_freq_shared(struct uag_gov_cpu *sg_cpu, u64 time)
{
	...
	sg_policy->multi_util[UA_UTIL_SYS] = util;
	if (sg_policy->tunables->multi_tl_enable) {
		fbg_freq_policy_util(sg_policy->flags, policy->cpus,
				&sg_policy->multi_util[UA_UTIL_FBG]);
		util = uag_multi_util_max(sg_policy);
	} else
...
static void android_vh_cpu_idle_enter_handler(void *unused, int *index, struct cpuidle_device *dev)
{
	...
	*/
	...
	/* some exception 2 frames calculate idle together */
