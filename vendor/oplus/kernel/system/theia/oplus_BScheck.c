...
static int black_screen_error_counter = 0;
static int black_screen_error_happened_flag = 0;
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static int black_screen_proc_open(struct inode *inode, struct file *file)
{
        int ret;
...
#else
static const struct proc_ops black_screen_check_proc_fops = {
	.proc_open = black_screen_proc_open,
	.proc_read = black_screen_check_proc_read,
	.proc_write = black_screen_check_proc_write,
};

static const struct proc_ops black_screen_report_proc_fops = {
	.proc_open = black_screen_proc_open,
	.proc_read = black_screen_report_proc_read,
	.proc_write = black_screen_report_proc_write,
};

static const struct proc_ops black_screen_switch_proc_fops = {
	.proc_open = black_screen_proc_open,
	.proc_read = black_screen_switch_proc_read,
	.proc_write = black_screen_switch_proc_write,
};
#endif
...
#else
static const struct proc_ops black_screen_check_status_proc_fops = {
	.proc_open = black_screen_proc_open,
	.proc_read = black_screen_check_status_proc_read,
};
#endif
...
static bool black_check_error_happened_before(struct black_data *bla_data)
{
	BLACK_DEBUG_PRINTK("error_happened before:%d\n", black_screen_error_happened_flag);
	return (black_screen_error_happened_flag > 0 ? true : false);
}
#endif
...
static int black_mark_or_clear_error_happened_flag(struct black_data *bla_data, int mark_or_clear)
{
	black_screen_error_happened_flag = mark_or_clear;
	return black_screen_error_happened_flag;
}
#endif
...
static int black_write_error_count(struct black_data *bla_data)
{
	bla_data->error_count_old = black_screen_error_counter;
	bla_data->error_count_new = bla_data->error_count_new + bla_data->error_count_old;
	black_screen_error_counter = bla_data->error_count_new;
	BLACK_DEBUG_PRINTK("black_error_count %d\n", black_screen_error_counter);
	return black_screen_error_counter;
}
#endif
...
static void dump_freeze_log(void)
{
    ...
    if(NULL != t_init)
    {
        ...
        send_sig_info(SIG_THEIA, SEND_SIG_PRIV, 0);
    }
}
...
#if IS_ENABLED(CONFIG_DRM_MSM) || IS_ENABLED(CONFIG_DRM_OPLUS_NOTIFY)
	ret = msm_drm_register_client(&g_black_data.fb_notif);
#else
	ret= fb_register_client(&g_black_data.fb_notif);
#endif
	if (ret) {
		printk("black_fb_notifier_callback register failed !\n");
