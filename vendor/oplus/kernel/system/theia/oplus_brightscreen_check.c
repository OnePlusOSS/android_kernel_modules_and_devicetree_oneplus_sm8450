...
//#define BLACKCREEN_COUNT_FILE	"/data/oplus/log/bsp/blackscreen_count.txt"
...
static int bright_screen_error_counter = 0;
static int bright_screen_error_happened_flag = 0;
#endif
struct bright_data{
	int is_panic;
	int status;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static int bright_screen_proc_open(struct inode *inode, struct file *file)
{
        int ret;
...
#else
static const struct proc_ops bright_screen_check_proc_fops = {
	.proc_open = bright_screen_proc_open,
	.proc_read = bright_screen_check_proc_read,
	.proc_write = bright_screen_check_proc_write,
};
#endif
...
#else
static const struct proc_ops bright_screen_check_status_proc_fops = {
	.proc_open = bright_screen_proc_open,
	.proc_read = bright_screen_check_status_proc_read,
};
#endif
...
static bool bright_check_error_happened_before(struct bright_data *bri_data)
{
	BRIGHT_DEBUG_PRINTK("error_happened before:%d\n", bright_screen_error_happened_flag);
	return (bright_screen_error_happened_flag > 0 ? true : false);
}

static int bright_mark_or_clear_error_happened_flag(struct bright_data *bla_data, int mark_or_clear)
{
	bright_screen_error_happened_flag = mark_or_clear;
	return bright_screen_error_happened_flag;
}

static int bright_write_error_count(struct bright_data *bri_data)
{
	bri_data->error_count_old = bright_screen_error_counter;
	bri_data->error_count_new = bri_data->error_count_new + bri_data->error_count_old;
	bright_screen_error_counter = bri_data->error_count_new;
	BRIGHT_DEBUG_PRINTK("black_error_count %d\n", bright_screen_error_counter);
	return bright_screen_error_counter;
}
#endif
...
#else
static const struct proc_ops bright_screen_switch_proc_fops = {
	.proc_open = bright_screen_proc_open,
	.proc_read = bright_screen_switch_proc_read,
...
#if IS_ENABLED(CONFIG_DRM_MSM) || IS_ENABLED(CONFIG_DRM_OPLUS_NOTIFY)
	ret = msm_drm_register_client(&g_bright_data.fb_notif);
#else
	ret = fb_register_client(&g_bright_data.fb_notif);
#endif
	if (ret) {
