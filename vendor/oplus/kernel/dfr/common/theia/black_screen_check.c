...
#define BLACK_SLOW_STATUS_TIMEOUT_MS    10000
	...
	struct panel_event_notification *notification, void *client_data)
{
	...
	case THEIA_PANEL_UNBLANK_EVENT:
		...
		break;
	default:
		BLACK_DEBUG_PRINTK("black_fb_notifier_callback: status:%d, blank:%d\n",
			g_black_data.status, g_black_data.blank);
		break;
	}
}
	...
	unsigned long event, void *data)
{
	...
	case THEIA_PANEL_BLANK_EVENT:
		...
		if (g_black_data.status != BLACK_STATUS_CHECK_DEBUG && g_black_data.blank == THEIA_PANEL_UNBLANK_VALUE) {
			delete_timer("FINISH_FB", true);
			BLACK_DEBUG_PRINTK("black_fb_notifier_callback: del timer, status:%d, blank:%d\n",
				g_black_data.status, g_black_data.blank);
		} else {
