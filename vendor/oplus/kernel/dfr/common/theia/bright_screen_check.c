	...
	struct panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_UNBLANK:
		...
		break;
	default:
		BRIGHT_DEBUG_PRINTK("bright_fb_notifier_callback: status:%d, blank:%d\n",
			g_bright_data.status, g_bright_data.blank);
		break;
	}
}
	...
	unsigned long event, void *data)
{
	...
	case THEIA_PANEL_BLANK_EVENT:
		...
		if (g_bright_data.status != BLACK_STATUS_CHECK_DEBUG && g_bright_data.blank == THEIA_PANEL_BLANK_VALUE) {
			delete_timer("FINISH_FB", true);
			BRIGHT_DEBUG_PRINTK("bright_fb_notifier_callback: del timer, status:%d blank:%d\n",
				g_bright_data.status, g_bright_data.blank);
		} else {
