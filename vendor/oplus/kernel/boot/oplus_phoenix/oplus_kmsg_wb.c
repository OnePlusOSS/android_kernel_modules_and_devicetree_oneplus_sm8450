...
#define TARGET_PARTITION "oplusreserve5"
/*oplusreserve5 partition dev is [8:93] on QC 8450*/
#define QC_8450_OPLUSRESERVE5_MAJOR 8
#define QC_8450_OPLUSRESERVE5_MINOR 93
...
/*
 * if you wanna use feature,
 * you need add your target partition major and minor number in dev_list
 */
const dev_t dev_list[] =
{
	MKDEV(QC_8450_OPLUSRESERVE5_MAJOR, QC_8450_OPLUSRESERVE5_MINOR) //QC 8450
};
...
RETRY_DEVICE:
	for (i = 0; i < ARRAY_SIZE(dev_list); i++) {
		bdev = blkdev_get_by_dev(dev_list[i], FMODE_READ | FMODE_WRITE, NULL);
		if (!IS_ERR(bdev)) {
			if (strcmp((char *)bdev->bd_part->info->volname, TARGET_PARTITION) == 0) {
